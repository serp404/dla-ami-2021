...
class GroupLengthBatchSampler(torch.utils.data.BatchSampler):
    def __init__(self, data_source, batch_size, group_size):
        ...
        argsort_ids = torch.argsort(torch.tensor([len(x['audio']) for x in data_source]))
    ...
    def __iter__(self):
        ...
        while num_yielded < self.data_len:
            ...
            current_group_size = torch.count_nonzero(self.id2group == current_group)
            random_ids = torch.randperm(current_group_size)[:self.batch_size]
            batch = indices[self.id2group == current_group][random_ids].tolist()

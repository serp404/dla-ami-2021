...
class GraphemeAligner(nn.Module):
    def __init__(self, melspec_sr: int):
        super().__init__()

        self._wav2vec2 = WAV2VEC2_ASR_BASE_960H.get_model()
        ...
        self._resampler = torchaudio.transforms.Resample(
            orig_freq=melspec_sr, new_freq=16_000
        )

    def _decode_text(self, text: str) -> torch.LongTensor:
    ...
    def forward(
        ...
        for index in range(batch_size):
            current_wav = wavs[index, :wavs_lengths[index]].unsqueeze(dim=0)
            ...
            relative_durations = torch.tensor(
                [s.length / num_frames for s in segments]
            )
    ...
    def _backtrack(self, trellis, emission, tokens, blank_id=0):
        ...
        for t in range(t_start, 0, -1):
            ...
            # Note (again):
            # `emission[J-1]` is the emission at time frame `J` of
            # trellis dimension.
    ...
    def _merge_repeats(self, text: str, path: str) -> tp.List[Segment]:
        i1, i2 = 0, 0
        segments = []
        while i1 < len(path):
            while i2 < len(path) and \
